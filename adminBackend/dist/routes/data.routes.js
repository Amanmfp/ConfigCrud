...
router.get("/models", async (req, res) => {
    try {
        const schemas = await SchemaDefinition_1.SchemaDefinition.find({}, { name: 1, label: 1, icon: 1 });
        res.json(schemas);
    }
    catch (err) {
...
router.get("/schema/:model", async (req, res) => {
    try {
        const schema = await SchemaDefinition_1.SchemaDefinition.findOne({ name: req.params.model });
        if (!schema)
...
router.get("/:model", async (req, res) => {
    try {
        const Model = await (0, dynamicModel_service_1.getDynamicModel)(req.params.model);
        if (!Model)
...
router.put("/:model/:id", async (req, res) => {
    try {
        ...
        if (!Model)
            return res.status(404).json({ error: `Model "${req.params.model}" not found` });
        const doc = await Model.findByIdAndUpdate(req.params.id, { ...req.body, updatedAt: new Date() }, { new: true, runValidators: true });
        if (!doc)
